...
class GPUTrainer:
    ...
    def prepare_data(self, limit_matches: int = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Prepare data for GPU training using series-level aggregation"""
        print("=== Preparing Series-Level Data for GPU Training ===")
        loader = EnhancedDataLoader()
        matches = loader.load_scraped_matches(limit=limit_matches)
        df = loader.create_series_level_dataset(matches, max_maps=3)
        if df.empty:
            raise ValueError("No data available for training")

        # Select features: kills, deaths, assists, agent, map for each map slot, plus context
        feature_columns = []
        for i in range(1, 4):
            feature_columns += [
                f'kills_map{i}', f'deaths_map{i}', f'assists_map{i}',
                # Optionally encode agent and map_name as categorical features later
            ]
        # Add context features
        feature_columns += ['team', 'opponent_team', 'tournament']
        # For now, drop non-numeric/categorical features (agent, map_name) or encode them as needed
        X = df[feature_columns].copy()
        # Simple encoding for categorical features (team, opponent_team, tournament)
        X = pd.get_dummies(X, columns=['team', 'opponent_team', 'tournament'], dummy_na=True)
        y = df['total_kills_first2']
